  ...
  return (
    <>
      <motion.div
        ...
        className={`relative group cursor-pointer rounded-2xl overflow-hidden glass-card-${party.color_theme} border-white/5 transition-all hover:border-white/20`}
        whileHover={{ y: -5, scale: 1.01 }}
      >
        <div className="p-4 flex items-center gap-4">
          {/* Small Avatar/Logo */}
          <div className="relative shrink-0">
            <div className={`w-16 h-16 rounded-full border-2 ${c.border} overflow-hidden bg-stone-800`}>
            ...
            <div className={`absolute -bottom-1 -right-1 w-7 h-7 rounded-lg border ${c.border} bg-stone-900 flex items-center justify-center overflow-hidden`}>
              {party.logo_url ? <img src={party.logo_url} className="w-full h-full object-cover" alt="" /> : <span className="text-[10px] font-bold">{party.number}</span>}
            </div>
          </div>
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className={`relative w-full max-w-lg overflow-hidden rounded-3xl shadow-2xl glass-card-${party.color_theme} border-white/20`}
            >
              {/* Close Button */}
              <button 
                onClick={() => setIsOpen(false)}
              ...
              <div className="relative">
                ...
                <div className="px-6 pb-8 -mt-16 relative z-10 text-center">
                  <div className="inline-block relative mb-4">
                    <div className={`w-32 h-32 md:w-40 md:h-40 rounded-full border-4 ${c.border} overflow-hidden bg-stone-800 shadow-xl mx-auto`}>
                      {party.president_photo_url ? (
                        <img src={party.president_photo_url} className="w-full h-full object-cover" alt="" />
                      ) : (
                    ...
                    <div className={`absolute bottom-0 right-0 w-12 h-12 md:w-16 md:h-16 rounded-2xl border-2 ${c.border} bg-stone-900 shadow-lg flex items-center justify-center overflow-hidden`}>
                       {party.logo_url ? <img src={party.logo_url} className="w-full h-full object-cover" alt="" /> : <span className="text-xl font-bold">{party.number}</span>}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-1 mb-6">
                    ...
                    <p className="text-stone-400 font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                  ...
                  {party.slogan && (
                    <div className="mb-8 p-6 bg-white/[0.03] rounded-2xl border border-white/5 italic text-stone-300 relative">
                      <Quote className="absolute -top-3 -left-1 opacity-20" size={32} />
